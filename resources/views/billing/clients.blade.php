    ...
    <div class="card-content collapse show">
      <div class="card-body">
        <div class="users-list-filter">
          <form id="frm-filter" action="{{ url('/billing/clients') }}" method="get">
            <div class="row">
              ...
              <div class="col-6 col-sm-4 col-lg-2">
                ...
                <fieldset class="form-group">
                  <select name="type" class="form-control" onchange="filterFrm()">
                    ...
                    <option <?php echo $type == 'AV_DIGITAL' ? 'selected' : ''; ?> value="AV_DIGITAL">AV_DIGITAL</option>
                                ...
                                @if($orders)                                  
                                  <tbody>
                                    @foreach($orders as $order)
                                      <tr>
                                        <td>{{ $order->employees_first_name }} {{ $order->employees_middle_name }} {{ $order->employees_last_name }}</td>
                                        <td>{{ $order->user_state }}</td>
                                        <td>{{ $order->task_display_id }}</td>
                                        <td>{{ date('d M Y', strtotime($order->received_date)) }}</td>
                                        <td>{{ $order->verification_date ? date('d M Y', strtotime($order->verification_date)) : '' }}</td>
                                      </tr>
                                    @endforeach
                                  </tbody>                                  
                                @endif
