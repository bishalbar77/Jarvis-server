          ...
          <div class="card-body">
            <div class="tab-content">
              <div class="tab-pane active" id="account" aria-labelledby="account-tab" role="tabpanel">
                ...
                <form novalidate action="{{ url('severity/update/'.$severity->id) }}" method="post" enctype="multipart/form-data" onsubmit="return validation()">
                  {{ csrf_field() }}
                    ...
                    <select name="task_type_id" id="task_type_id" class="select2 form-control">
                      ...
                      @if($tasktypes)
                        @foreach($tasktypes as $tasktype)
                          <option {{ $severity->task_type_id == $tasktype->id ? 'selected' : '' }} value="{{ $tasktype->id }}">{{ $tasktype->task_type }}</option>
                        @endforeach
                      @endif
                    </select>                      
                    @if($errors->has('name'))
                  ...
                  <div class="form-group">
                    ...
                    <select name="task_severity" id="task_severity" class="select2 form-control">
                      <option value="">Select</option>
                      <option {{ $severity->task_severity == 'GREEN' ? 'selected' : '' }} value="GREEN">GREEN</option>
                      <option {{ $severity->task_severity == 'YELLOW' ? 'selected' : '' }} value="YELLOW">YELLOW</option>
                      <option {{ $severity->task_severity == 'RED' ? 'selected' : '' }} value="RED">RED</option>
                    </select>
                    ...
                    <select name="severity_message_id" id="severity_message_id" class="select2 form-control">
                      ...
                      @if($messeges)
                        @foreach($messeges as $messege)
                          <option {{ $severity->severity_message_id == $messege->id ? 'selected' : '' }} value="{{ $messege->id }}">{{ $messege->id }} -- {{ $messege->severity_message }}</option>
                        @endforeach
                      @endif
                    </select>
