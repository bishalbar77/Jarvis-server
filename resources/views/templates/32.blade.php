...
@php
    function changeText($str){
        $data = $str;
        if($str == 'PARTIAL_MATCHED'){
            $data = 'UNABLE TO VERIFY';
        }
        return $data;
    }
@endphp
        ...
        <div class="top-part">
            <div class="justify-content-center align-items-center">
                <div class="col-lg-8 col-sm-8 col-8" style="margin-top: -20px; width: 65%; float: left; padding-top: 50px;">
                    <h4>Education Verification Report</h4>
        ...
        <div class="row">
            ...
            <div class="col-12" style="padding-top: 70px; padding-bottom: -40px;">
                ...
                <div style="width: 97%; padding-left: -15px !important; margin-bottom: 10px;">
                    <h4 id="h4" style="margin-bottom: -3px; font-size: 15px;">{{ $verificationFields['antecedants_data']['cource_name']['name'] ?? '' }}</h4>
                    @php
                        $addrmatchstatus = $verificationFields['antecedants_data']['cource_name']['match_status'] ?? '';
                        if($addrmatchstatus == 'NO_MATCHED'){
                            $addrmatchstatus = 'NO_MATCH_FOUND';
                        }
                    @endphp
                    <span id="h5-c" style="float: right; text-transform: uppercase; color: 
                    @if($verificationFields['antecedants_data']['cource_name']['match_status'] == 'MATCHED')
                        #6cc077;
                    @elseif($verificationFields['antecedants_data']['cource_name']['match_status'] == 'PARTIAL_MATCHED')
                        #ffd600;
                    @else                        
                        #FC1102;
                    @endif
                    " class="antecedents-value {{ str_replace(' ', '_', $verificationFields['antecedants_data']['cource_name']['match_status']) }}">{{ str_replace('_', ' ', changeText($addrmatchstatus)) }}</span>
                    <h5 id="p">{{ $verificationFields['antecedants_data']['cource_name']['value'] ?? '' }}</h5>
                </div>

                <div style="width: 97%; padding-left: -15px !important; margin-bottom: 10px;">
                    <h4 id="h4" style="margin-bottom: -3px; font-size: 15px;">{{ $verificationFields['antecedants_data']['edu_passing_year']['name'] ?? '' }}</h4>
                    @php
                        $addrmatchstatus = $verificationFields['antecedants_data']['edu_passing_year']['match_status'] ?? '';
                        if($addrmatchstatus == 'NO_MATCHED'){
                            $addrmatchstatus = 'NO_MATCH_FOUND';
                        }
                    @endphp
                    <span id="h5-c" style="float: right; text-transform: uppercase; color: 
                    @if($verificationFields['antecedants_data']['edu_passing_year']['match_status'] == 'MATCHED')
                        #6cc077;
                    @elseif($verificationFields['antecedants_data']['edu_passing_year']['match_status'] == 'PARTIAL_MATCHED')
                        #ffd600;
                    @else                        
                        #FC1102;
                    @endif
                    " class="antecedents-value {{ str_replace(' ', '_', $verificationFields['antecedants_data']['edu_passing_year']['match_status']) }}">{{ str_replace('_', ' ', changeText($addrmatchstatus)) }}</span>
                    <h5 id="p">{{ $verificationFields['antecedants_data']['edu_passing_year']['value'] ?? '' }}</h5>
                </div>

                <div style="width: 97%; padding-left: -15px !important; margin-bottom: 10px;">
                    <h4 id="h4" style="margin-bottom: -3px; font-size: 15px;">{{ $verificationFields['antecedants_data']['edu_roll_no']['name'] ?? '' }}</h4>
                    @php
                        $addrmatchstatus = $verificationFields['antecedants_data']['edu_roll_no']['match_status'] ?? '';
                        if($addrmatchstatus == 'NO_MATCHED'){
                            $addrmatchstatus = 'NO_MATCH_FOUND';
                        }
                    @endphp
                    <span id="h5-c" style="float: right; text-transform: uppercase; color: 
                    @if($verificationFields['antecedants_data']['edu_roll_no']['match_status'] == 'MATCHED')
                        #6cc077;
                    @elseif($verificationFields['antecedants_data']['edu_roll_no']['match_status'] == 'PARTIAL_MATCHED')
                        #ffd600;
                    @else                        
                        #FC1102;
                    @endif
                    " class="antecedents-value {{ str_replace(' ', '_', $verificationFields['antecedants_data']['edu_roll_no']['match_status']) }}">{{ str_replace('_', ' ', changeText($addrmatchstatus)) }}</span>
                    <h5 id="p">{{ $verificationFields['antecedants_data']['edu_roll_no']['value'] ?? '' }}</h5>
                </div>

                <div style="width: 97%; padding-left: -15px !important; margin-bottom: 10px;">
                    <h4 id="h4" style="margin-bottom: -3px; font-size: 15px;">{{ $verificationFields['antecedants_data']['edu_school_name']['name'] ?? '' }}</h4>
                    @php
                        $addrmatchstatus = $verificationFields['antecedants_data']['edu_school_name']['match_status'] ?? '';
                        if($addrmatchstatus == 'NO_MATCHED'){
                            $addrmatchstatus = 'NO_MATCH_FOUND';
                        }
                    @endphp
                    <span id="h5-c" style="float: right; text-transform: uppercase; color: 
                    @if($verificationFields['antecedants_data']['edu_school_name']['match_status'] == 'MATCHED')
                        #6cc077;
                    @elseif($verificationFields['antecedants_data']['edu_school_name']['match_status'] == 'PARTIAL_MATCHED')
                        #ffd600;
                    @else                        
                        #FC1102;
                    @endif
                    " class="antecedents-value {{ str_replace(' ', '_', $verificationFields['antecedants_data']['edu_school_name']['match_status']) }}">{{ str_replace('_', ' ', changeText($addrmatchstatus)) }}</span>
                    <h5 id="p">{{ $verificationFields['antecedants_data']['edu_school_name']['value'] ?? '' }}</h5>
                </div>
